...
export async function GET(
  ...
  try {
    ...
    // Verify keyword belongs to user's profile through adGroup->campaign->profile
    const keyword: any = await prisma.keyword.findFirst({
      where: {
        id,
        adGroup: {
          campaign: {
    ...
    // Find search terms where keywordId matches
    ...
    return NextResponse.json({
      ...
      searchTerms: aggregated,
    });
  } catch (error) {
    console.error('Error fetching keyword search terms:', error);
    return NextResponse.json(
